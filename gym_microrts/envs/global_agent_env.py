...
class GlobalAgentEnv(BaseSingleAgentEnv):
    ...
    def step(self, action, raw=False, customize=False):
        ...
        """
        # obs[3].clip(max=1) - obs[4].clip(max=1) means mask busy units
        # obs[3]代表該像素點為哪個單位，obs[4]代表該單位正在執行的動作。
        # 若指定像素點的單為不為"無"，執行的動作也不為"無"，則obs[3].clip(max=1) - obs[4].clip(max=1)必定大於1，代表該單位在執行動作中(忙碌)。
        
        
        # * np.where((obs[2])==2,0, (obs[2]))) 產生一個與地圖大小相同的mask，我方單位處為1，其餘為0
        # * np.where((obs[2])==1,0, (obs[2]).clip(max=1))).flatten() 產生一個與地圖大小相同的mask，敵方單位處為1，其餘為0
        """
